 ...
 * 
 */
define("ytgMatch/scripts/simulatedStocks/myPortfolio/buyStock", function(require, exports, module) {
	var appUtils = require("appUtils"), 
	    layerUtils = require("layerUtils"), 
        gconfig = require("gconfig"),
        validator = require("validatorUtil"),
        ytgMatchService = require("ytgMatchService"),//服务
		pageCode = "simulatedStocks/myPortfolio/buyStock", 
		_pageId = "#simulatedStocks_myPortfolio_buyStock ";
	...
	var stock_name = '';
	var market_id = '';
	var isLastDay ="";//判断是否是比赛最后的交易日
	var parentPageCode ="";//前置页面
	/**
	 * 初始化
	 */
	function init() {
		...
		isLastDay = appUtils.getSStorageInfo("isLastDay");
		parentPageCode = appUtils.getSStorageInfo("parentPageCode");
		//设置输入框的长度
	...
	function bindPageEvent() {
		...
		appUtils.bindEvent($(_pageId + ".icon_back"), function(e) {
			if (parentPageCode) {
				appUtils.pageInit(pageCode,parentPageCode);
			}else{
		...
		appUtils.bindEvent($(_pageId + ".login_btn"), function(e) {
			if (isLastDay =="0") {
				...
				layerUtils.iMsg(-1,"比赛最后一个交易日不能买入股票");
			}
	...
	//将组合持仓填充到页面
	function fillPositionData(results)
	{
		var result =results;
		$(_pageId + "#position").siblings().remove();
	...
	function initVIScroll(){
		positionHeight= $(window).height() - $(_pageId +" .header").height()-$(_pageId+" .top_nav").height() -$(_pageId + ".stock_box_list").height()-12;
...
//					"visibleHeight":
	...
	function stockAutoInput(){
		...
		ytgMatchService.stockAutoInput(param,function callBack(resultVo){
			if(Number(resultVo.error_no) == 0){
	...
	function queryStockInfo(){
		...
		ytgMatchService.queryStockInfo(param,function callBack(resultVo){
			if(Number(resultVo.error_no) == 0){
	...
	//计算最大可买
	function getMaxBuy(price){
		//计算最大可买
		var userable_balance = appUtils.getSStorageInfo("userableBalance"); //获取可用资金
		var max_buy =Math.floor(userable_balance/price);
		if(price == "0.0"){
			max_buy=0; //最大可买
		}else{
			max_buy =(Math.floor(max_buy/100))*100;
		}
		$(_pageId+"#maxBuy").html(max_buy); //最大可买
	...
	var userSpace_myPortfolio_buyStock = {
		"init" : init,
	...
	module.exports = userSpace_myPortfolio_buyStock;
});