 ...
 * @date 2016-03-16 （改）
 */
define("ytgMatch/scripts/simulatedStocks/myPortfolio/cancelEntrust", function(require, exports, module) {
	var appUtils = require("appUtils"), 
	    layerUtils = require("layerUtils"), 
        gconfig = require("gconfig"),
        ytgMatchService = require("ytgMatchService"),//服务
		
		pageCode = "simulatedStocks/myPortfolio/cancelEntrust", 
		_pageId = "#simulatedStocks_myPortfolio_cancelEntrust ";
	...
	function getTodayEntrust(insertType){
		...
		ytgMatchService.queryTodayEntrust(param,function callBack(resultVo){
	...
	function fillEntrustData(result,insertType)
	{
		...
		for(var i = 0;i<result.length;i++)
		{
				...
				//不现实已成数据
				if (result[i].trade_status_name !="已成" && result[i].trade_status_name !="撤单成功" ) {
					...
					html += '<li><span>状态</span>'+result[i].trade_status_name+'</li>';
					html += '</ul></div>';
					html += '</div>';
					html += '</div></div>';
				}
            ...
            if (html =="") {
            	var height = $(window).height() - $(_pageId +" .header").height()-$(_pageId+" .top_nav").height();
				$(_pageId + "#fillDataList").html("");
				html = '<div  style="height:'+height+'px; font-size:16px; color:#9999B1; background-color:#FFFFFF; text-align:center; padding-top:150px; margin-top: 0.1rem;">您暂无可撤委托</div>';
	...
	function cancelEntrust(cancel_id){
		...
		ytgMatchService.cancelTradeOrder(param,function callBack(resultVo){
			if(Number(resultVo.error_no) == 0){
