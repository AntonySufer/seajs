 ...
 * @date  2016-03-16
 */
define("ytgMatch/scripts/simulatedStocks/myPortfolio/queryEntrust/todayDeal", function(require, exports, module) {
	var appUtils = require("appUtils"), 
	    layerUtils = require("layerUtils"), 
        gconfig = require("gconfig"),
        ytgMatchService = require("ytgMatchService"),//服务
		
		pageCode = "simulatedStocks/myPortfolio/queryEntrust/todayDeal", 
		_pageId = "#simulatedStocks_myPortfolio_queryEntrust_todayDeal ";
	...
	//	pageCommon.headerMenuFunc(_pageId,pageCode);
		...
		appUtils.bindEvent($(_pageId + "#todayEntrust"), function(e) {
			...
			appUtils.pageInit(pageCode,"simulatedStocks/myPortfolio/queryEntrust/todayEntrust");
		});
	}
	 ...
	 * 今日成交
	 * */
	function getTodayDeal(loadTypes){
		 decount++;
		    var type;
	    	if(decount>1){
	    		type=false;
		...
		ytgMatchService.queryTodayDeal(param,function callBack(resultVo){
			if(Number(resultVo.error_no) == 0){
				//layerUtils.iMsg(0,"当日成交查询成功");
				var result = resultVo.results[0].data;
				if (result != null && result.length>0) {
					fillEntrustData(result,loadTypes);
					num = result.length;
	...
	//将历史成交数据填充到页面
	function fillEntrustData(result,loadType)
	{
	
		var html = "";
		if (loadType!="append") {
			$(_pageId + ".entrust_list").html(html);
		}
