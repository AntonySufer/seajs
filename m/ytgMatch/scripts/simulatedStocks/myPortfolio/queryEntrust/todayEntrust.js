 ...
 * @date 2016-03-16 （改）
 */
define("ytgMatch/scripts/simulatedStocks/myPortfolio/queryEntrust/todayEntrust", function(require, exports, module) {
	var appUtils = require("appUtils"), 
	    layerUtils = require("layerUtils"), 
        gconfig = require("gconfig"),
        ytgMatchService = require("ytgMatchService"),//服务
		
		pageCode = "simulatedStocks/myPortfolio/queryEntrust/todayEntrust", 
		_pageId = "#simulatedStocks_myPortfolio_queryEntrust_todayEntrust ";
	...
	function fillEntrustData(result,insertType)
	{
		...
		for(var i = 0;i<result.length;i++)
		{
			...
			html += '<li><span>金额</span>'+(result[i].order_price * result[i].order_qty).toFixed(2)+'</li>';
			html += '</ul>';
			html += '</div>';
			html += '</div>';
		}
